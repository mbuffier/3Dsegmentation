...
{
	// if there is 4 arguments, the semantic image is added 
	std::cout << "argc : " << argc << std::endl ;
	if (argc==5) isSemantic = true ;
	...
	//****** new interface *******
	winSize.x = (int)(2.5f * (float)(imageSource->getDepthImageSize().x));
	winSize.y = imageSource->getDepthImageSize().y + textHeight;

	float h1 = textHeight / (float)winSize.y;  
	float h2 = (1.f + h1) /2;

	winReg[0] = Vector4f(0.0f, h1, 0.4f, 1.0f);   // Main render

	winReg[1] = Vector4f(0.4f, h2, 0.7f, 1.0f);   // bottom  left 
	winReg[2] = Vector4f(0.4f, h1, 0.7f, h2);     // top left 
	winReg[3] = Vector4f(0.7f, h2, 1.0f, 1.0f);     // bottom right
	winReg[4] = Vector4f(0.7f, h1, 1.0f, h2);     // top right 
	...
	// add the image with the semantic information on the imageSource 
	if (isSemantic) inputSemanticImage = new ITMShortImage(imageSource->getDepthImageSize(), true, allocateGPU);

	// inputIMUMeasurement = new ITMIMUMeasurement();
	saveImage = new ITMUChar4Image(imageSource->getDepthImageSize(), true, false);

	// ***** decide what to see in the window ******
	outImageType[0] = ITMMainEngine::InfiniTAM_IMAGE_SCENELABELEDRAYCAST; // result 
	
	// to visualize the propagation 
	outImageType[1] = ITMMainEngine::InfiniTAM_IMAGE_NORMAL_LABELED	; // top left  
	outImageType[2] = ITMMainEngine::InfiniTAM_IMAGE_SEGMENTATION_MODEL ; // bottom left 
	outImageType[3] = ITMMainEngine::InfiniTAM_IMAGE_PROPAGATION_MODEL; //  top right 
	outImageType[4] = ITMMainEngine::InfiniTAM_IMAGE_SEMANTIC_SEG ;  // bottom right // InfiniTAM_IMAGE_SEMANTIC_SEG InfiniTAM_IMAGE_NORMAL_SEGMENTED

	// to debug the comparaison between vertices and normals  
	/*outImageType[1] = ITMMainEngine::InfiniTAM_IMAGE_NORMAL_SEGMENTED	; // top left 
	outImageType[2] = ITMMainEngine::InfiniTAM_IMAGE_NORMAL_LABELED; // bottom left
	outImageType[3] = ITMMainEngine::InfiniTAM_IMAGE_ORIGINAL_DEPTH ; //  top right
	outImageType[4] = ITMMainEngine::InfiniTAM_IMAGE_NORMAL ; */ // bottom right 
...
{
	ITMUChar4Image screenshot(getWindowSize(), true, false, false);
...
void UIEngine::ProcessFrame()
{
	...
	// ******* added to handle the add of semantic *********$ 
	if (isSemantic) {imageSource->getAllImages(inputRGBImage, inputRawDepthImage, inputSemanticImage);}
	else imageSource->getImages(inputRGBImage, inputRawDepthImage) ;
	...
	if (isSemantic) mainEngine->ProcessAllFrame(inputRGBImage, inputRawDepthImage, inputSemanticImage) ;
	else if (imuSource != NULL && isSemantic == false) mainEngine->ProcessFrame(inputRGBImage, inputRawDepthImage, inputIMUMeasurement);
	else mainEngine->ProcessFrame(inputRGBImage, inputRawDepthImage);

	// ***** ADDED TO SAVE IMAGES *****
	char str[250];
	// all result window
	/*sprintf(str, "coFusion/out/%04d.ppm", currentFrameNo);
	SaveScreenshot(str) ;*/

	// one image  
	/*sprintf(str, "seq_02/eva_WS/%04d.ppm", currentFrameNo);
	ITMUChar4Image *out = new ITMUChar4Image(inputRGBImage->noDims, true, false); //InfiniTAM_IMAGE_NORMAL_SEGMENTED
	mainEngine->GetImage(out,ITMMainEngine::InfiniTAM_IMAGE_SEGMENTATION_MODEL, &(this->freeviewPose), &(this->freeviewIntrinsics)) ;
	SaveImageToFile(out, str);
	delete out ; */
	
	/*sprintf(str, "coFusion/frameWiseSeg/%04d.ppm", currentFrameNo);
	ITMUChar4Image *out = new ITMUChar4Image(inputRGBImage->noDims, true, false); //InfiniTAM_IMAGE_NORMAL_SEGMENTED
	mainEngine->GetImage(out,ITMMainEngine::InfiniTAM_IMAGE_NORMAL_LABELED, &(this->freeviewPose), &(this->freeviewIntrinsics)) ;	
	SaveImageToFile(out, str); */
