...
static void CreateDefaultImageSource(ImageSourceEngine* & imageSource, IMUSourceEngine* & imuSource, const char *arg1, const char *arg2, const char *arg3, const char *arg4)
{
	...
	if (filename2 != NULL)
	{
		if (filename3 != NULL)
		{
			printf("using rgb images: %s\nusing depth images: %s\nusing semantic segmentation images: %s\n", filename1, filename2, filename3);
			imageSource = new ImageFileReader(calibFile, filename1, filename2, filename3) ; // add the segmentation folder 
		}
		else
		{
			// as usual 
...
try
{
	...
	do {
		...
		++arg;
		if (argv[arg] != NULL) arg4 = argv[arg]; else break; // segmentation images 
	} while (false);
