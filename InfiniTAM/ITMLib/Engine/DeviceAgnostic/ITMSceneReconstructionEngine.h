...
template<class TVoxel>
_CPU_AND_GPU_CODE_ inline void computeUpdatedVoxelLabelInfo(DEVICEPTR(TVoxel) &voxel, const THREADPTR(Vector4f) & pt_model, 
		const THREADPTR(Matrix4f) & M_rgb, const THREADPTR(Vector4f) & projParams_rgb,
		const CONSTPTR(int)* labelMap, const CONSTPTR(Vector2i) & imgSize)
{
	Vector4f pt_camera; Vector2f pt_image;

	// look the point in the image frame
	pt_camera = M_rgb * pt_model;
	pt_image.x = projParams_rgb.x * pt_camera.x / pt_camera.z + projParams_rgb.z;
	pt_image.y = projParams_rgb.y * pt_camera.y / pt_camera.z + projParams_rgb.w;

	// std::cout << "pt image : " << pt_image << std::endl ;

	// the voxel isn't in the image 
	if ((pt_image.x < 3) || (pt_image.x > imgSize.x - 4) || (pt_image.y < 3) || (pt_image.y > imgSize.y - 4)) {
		voxel.label = 0 ;
		return;
	}
	int maplabel = labelMap[int(round(pt_image.x) + round(pt_image.y)*float(imgSize.x))] ; 

	if (maplabel==0) { // if the label is zero on the map, we will at the neighbors to give a label 
		maplabel = neighborLabel(labelMap,pt_image, imgSize) ;
	}

	// find the color for this pixel 
	Vector3u color ;

    int test1 = ((maplabel & 0xff) << 5) % 256 ; //& 0xff;
    int test2 = ((maplabel & 0xff) << 1) % 256 ; //& 0xff;
    int test3 = ((maplabel & 0xff) << 3) % 256 ; // 0xff;
	...
	if (voxel.confidence==-1) {
		// unlabeled : we label it, initialize confidence and give it a gray value
		voxel.label = maplabel ;  
		voxel.confidence = 0 ;
		voxel.clr = Vector3u((uchar)(128), (uchar)(128), (uchar)(128)) ;
		return ;
	}
	else if (voxel.label == maplabel) {
		voxel.confidence = std::min(10, voxel.confidence+1) ; // incrementation 
		voxel.clr =color ;
	}
	else if (voxel.label != maplabel) {
		voxel.confidence = std::max(0, voxel.confidence-1) ; // decrementation 
		if (voxel.confidence == 0) {
			voxel.label = maplabel ; // if zero -> new label 
			voxel.clr =color ;
		}
	}
	
	/*if (voxel.confidence > 2 ) {
		voxel.clr =color ; // new color 
	}
	else voxel.clr = Vector3u((uchar)(128), (uchar)(128), (uchar)(128)) ;*/
}
...
struct ComputeUpdatedVoxelInfo<true, TVoxel> {
		...
		const CONSTPTR(Vector4u) *rgb, const THREADPTR(Vector2i) & imgSize_rgb)
	{
		...
		// here, need to create the function to propagate the colors into the model from the Propagated labels map
		computeUpdatedVoxelColorInfo(voxel, pt_model, M_rgb, projParams_rgb, mu, maxW, eta, rgb, imgSize_rgb);
	}
};

template<class TVoxel>
_CPU_AND_GPU_CODE_ static void compute_label(DEVICEPTR(TVoxel) & voxel, const THREADPTR(Vector4f) & pt_model,
	const THREADPTR(Matrix4f) & M_d, const THREADPTR(Vector4f) & projParams_d,
	float mu, int maxW, const CONSTPTR(float) *depth, const CONSTPTR(Vector2i) & imgSize_d,
	const CONSTPTR(int) *labelMap)
{
	float eta = computeUpdatedVoxelDepthInfo(voxel, pt_model, M_d, projParams_d, mu, maxW, depth, imgSize_d);
	if ((eta > mu) || (fabs(eta / mu) > 0.25f)) return;

	// here, need to create the function to propagate the colors into the model from the Propagated labels map
	computeUpdatedVoxelLabelInfo(voxel, pt_model,M_d, projParams_d, labelMap, imgSize_d) ;
}
