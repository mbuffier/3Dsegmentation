...
template<class TVoxel, class TIndex>
_CPU_AND_GPU_CODE_ inline bool castRayColor(DEVICEPTR(Vector4f) &pt_out,DEVICEPTR(int) &lab_out, DEVICEPTR(int) &conf_out, int x, int y, const CONSTPTR(TVoxel) *voxelData,
	const CONSTPTR(typename TIndex::IndexData) *voxelIndex, Matrix4f invM, Vector4f projParams, float oneOverVoxelSize, 
	float mu, const CONSTPTR(Vector2f) & viewFrustum_minmax, bool allLabel)
{
	Vector4f pt_camera_f; Vector3f pt_block_s, pt_block_e, rayDirection, pt_result;
	bool pt_found, hash_found;
	float sdfValue = 1.0f;
	...
	// find the label and the confidence of this voxel 
	int label = read_labelFromVoxel(voxelData, voxelIndex, pt_result, hash_found, cache) ;
	lab_out = label ;

	// to add a part to obtain a mesh less noisy 
	/*if (allLabel) lab_out = label ;
	else {
		conf_out = read_confFromVoxel(voxelData, voxelIndex, pt_result, hash_found, cache) ;
	
		// try to test the value of conf_out 
		if (conf_out > 0 && conf_out < 2) lab_out = -1 ;
		else if (conf_out > 1) lab_out = label ;
	}*/
...
// draw the color pixel infonction of the label ! 
_CPU_AND_GPU_CODE_ inline void drawPixel_color(DEVICEPTR(Vector4u) & dest, const THREADPTR(float) & angle, int label, int conf)
{
	Vector4u outRes ;
	if (conf >= 0 && conf < 3) {
		outRes.x = (uchar)(0.8f * angle + 0.2f) ;
		outRes.y = (uchar)(0.8f * angle + 0.2f) ;
		outRes.z = (uchar)(0.8f * angle + 0.2f) ;
		outRes.w = (uchar)1.0f ;
	}
	else {
		outRes.x = (uchar)(0.8f * angle + 0.2f) * float(label % 256) ;
		outRes.y = (uchar)(0.8f * angle + 0.2f) * float((label/256) % 256) ;
		outRes.z = (uchar)(0.8f * angle + 0.2f) * float((label/256)/256 % 256) ;
		outRes.w = (uchar)1.0f ;
	}
...
//************ Proccess the pixel in the image to deal with the associate label in the model ********
template<bool useSmoothing>
_CPU_AND_GPU_CODE_ inline void processPixelICPColor(DEVICEPTR(Vector4u) *outRendering, DEVICEPTR(int) *labelMap, DEVICEPTR(int) *confMap, DEVICEPTR(Vector4f) *pointsMap, 
	DEVICEPTR(Vector4f) *normalsMap, const CONSTPTR(Vector4f) *pointsRay, const THREADPTR(Vector2i) &imgSize, 
	const THREADPTR(int) &x, const THREADPTR(int) &y, float voxelSize, const THREADPTR(Vector3f) &lightSource)
{
	Vector3f outNormal;
	float angle;
	...
	if (foundPoint)
	{
		drawPixel_color(outRendering[locId], angle,labelMap[locId], confMap[locId]);
