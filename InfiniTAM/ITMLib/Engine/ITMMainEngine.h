	...
	namespace Engine
	{
		...
		*/
		...
		private:
			...
			// to perform merging 
			std::map<std::pair<int, int>, int> mergingRecord ;
		...
		public:
			enum GetImageType
			{
				...
				// from the image analisis
				InfiniTAM_IMAGE_NORMAL,
				InfiniTAM_IMAGE_VERTEX,
				InfiniTAM_IMAGE_NORMAL_SEGMENTED, // 0 for edges, 1 otherwise 
				InfiniTAM_IMAGE_EDGE_PROBA,
				InfiniTAM_IMAGE_NORMAL_LABELED, // labeled version of InfiniTAM_IMAGE_NORMAL_SEGMENTED

				// from the model analisis
				InfiniTAM_IMAGE_NORMAL_MODEL, 
				InfiniTAM_IMAGE_VERTEX_MODEL,
				InfiniTAM_IMAGE_SEGMENTATION_MODEL, // projection of the model into camera frame 

				// from the semantic segmentation 
				InfiniTAM_IMAGE_SEMANTIC_SEG,

				// result of the algorithm, create the map which will update the model  
				InfiniTAM_IMAGE_PROPAGATION_MODEL, 

				// result final of the algorithm 
				InfiniTAM_IMAGE_SCENELABELEDRAYCAST,
			...
			void ProcessAllFrame(ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage, ITMShortImage *semImage) ;
			...
			// to get a float image to save directly 
			void GetFloatImage(ITMFloatImage *out, GetImageType getImageType, ITMPose *pose = NULL, ITMIntrinsics *intrinsics = NULL) ;
