...
// objectif -> generate an image in the renderState using the castRay function in ITMVisualisationEngine.h
template<class TVoxel, class TIndex> 
...
template<class TVoxel, class TIndex> 
static void GenericRaycast_Color(const ITMScene<TVoxel,TIndex> *scene, const Vector2i& imgSize, const Matrix4f& invM, Vector4f projParams, const ITMRenderState *renderState)
{
	projParams.x = 1.0f / projParams.x;
	projParams.y = 1.0f / projParams.y;
	...
	Vector4f *pointsRay = renderState->raycastResult->GetData(MEMORYDEVICE_CPU);
	int *labelsRay = renderState->labelResult->GetData(MEMORYDEVICE_CPU);
	int *confidencesRay = renderState->confidence->GetData(MEMORYDEVICE_CPU);
...
// ***************************** Create the image from the scene here ************
template<class TVoxel, class TIndex> 	
static void CreateICPMapsColor_common(const ITMScene<TVoxel,TIndex> *scene, const ITMView *view, ITMTrackingState *trackingState, ITMRenderState *renderState)
{
	Vector2i imgSize = renderState->raycastResult->noDims;
	Matrix4f invM = trackingState->pose_d->GetInvM();

	// now the renderState contains a labelMap 
	GenericRaycast_Color(scene, imgSize, invM, view->calib->intrinsics_d.projectionParamsSimple.all, renderState);
	trackingState->pose_pointCloud->SetFrom(trackingState->pose_d);
	...
	// label map
	int *label = renderState->labelResult->GetData(MEMORYDEVICE_CPU);
	int *confidence = renderState->confidence->GetData(MEMORYDEVICE_CPU);
	...
	for (int y = 0; y < imgSize.y; y++) for (int x = 0; x < imgSize.x; x++)
		// processPixelICP in visualisationEngine Device agnnostic
		processPixelICPColor<true>(outRendering, label, confidence, pointsMap, normalsMap, pointsRay, imgSize, x, y, voxelSize, lightSource);
}
...
// with the color 
template<class TVoxel, class TIndex>
void ITMVisualisationEngine_CPU<TVoxel,TIndex>::CreateICPMapsWithColor(const ITMView *view, ITMTrackingState *trackingState, ITMRenderState *renderState) const
{
	CreateICPMapsColor_common(this->scene, view, trackingState, renderState);
}
...
template<class TVoxel>
void ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHash>::CreateICPMapsWithColor(const ITMView *view, ITMTrackingState *trackingState, ITMRenderState *renderState) const
{
	CreateICPMapsColor_common(this->scene, view, trackingState, renderState);
}
