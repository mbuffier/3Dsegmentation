...
ITMMainEngine::ITMMainEngine(const ITMLibSettings *settings, const ITMRGBDCalib *calib, Vector2i imgSize_rgb, Vector2i imgSize_d)
{
	...
	mesh = NULL;
	//std::cout << "mesh : " << createMeshingEngine << std::endl ;
...
void ITMMainEngine::SaveSceneToMesh(const char *objFileName)
{
	...
	mesh->WriteOBJ(objFileName); // to save the color 
}
...
void ITMMainEngine::ProcessFrame(ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage, ITMIMUMeasurement *imuMeasurement)
{
	...
	// tracking the camera motion (modify the position of the camera)
	trackingController->Track(trackingState, view);

	// fusion -> here are the modification for the label propagation in denseMapper.cpp
	if (fusionActive) denseMapper->ProcessFrame(view, trackingState, scene, renderState_live, mergingRecord);

	// raycast to renderState_live for tracking and free visualisation
	trackingController->Prepare(trackingState, view, renderState_live);
}

// called when there is a semantic segmentation image 
void ITMMainEngine::ProcessAllFrame(ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage, ITMShortImage *semImage)
{
	// prepare image and turn it into a depth image
	viewBuilder->UpdateAllView(&view, rgbImage, rawDepthImage, semImage, settings->useBilateralFilter,settings->modelSensorNoise);
...
void ITMMainEngine::GetFloatImage(ITMFloatImage *out, GetImageType getImageType, ITMPose *pose, ITMIntrinsics *intrinsics) {
	if (view == NULL) return;
	...
	switch (getImageType)
	{
	case ITMMainEngine::InfiniTAM_IMAGE_EDGE_PROBA:
	{
		out->ChangeDims(view->depth->noDims) ;
		out->SetFrom(view->depthSegFloat, ORUtils::MemoryBlock<float>::CPU_TO_CPU);
		break;
	}
	};
}
...
void ITMMainEngine::GetImage(ITMUChar4Image *out, GetImageType getImageType, ITMPose *pose, ITMIntrinsics *intrinsics)
{
	...
	case ITMMainEngine::InfiniTAM_IMAGE_SEMANTIC_SEG :
		out->ChangeDims(view->depth->noDims) ;
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::ShortToUchar4(out, view->sem_seg);
		break;
	...
	case ITMMainEngine::InfiniTAM_IMAGE_SCENELABELEDRAYCAST:
	{
		out->ChangeDims(view->depth->noDims) ;
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::IntToUchar4(out,renderState_live->labelResult);
		break;
	}
	...
	case ITMMainEngine::InfiniTAM_IMAGE_UNKNOWN:
		break;
	// add a case to create the normal 
	case ITMMainEngine::InfiniTAM_IMAGE_NORMAL:
	{
		out->ChangeDims(view->depth->noDims);
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::NormalToUchar4(out, view->depthNormal);
		break;
	}
	case ITMMainEngine::InfiniTAM_IMAGE_EDGE_PROBA:
	{
		out->ChangeDims(view->depth->noDims);
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::WeightToUchar4(out, view->depthSegFloat);
		break;
	}
	case ITMMainEngine::InfiniTAM_IMAGE_VERTEX:
	{
		out->ChangeDims(view->depth->noDims);
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::NormalToUchar4(out, view->vertex);
		break;
	}
	case ITMMainEngine::InfiniTAM_IMAGE_NORMAL_SEGMENTED:
	{
		out->ChangeDims(view->depth->noDims) ;
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::BinaryToUchar4(out, view->depthSegmented);
		break;
	}
	case ITMMainEngine::InfiniTAM_IMAGE_NORMAL_LABELED:
	{
		out->ChangeDims(view->depth->noDims) ;
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::IntToUchar4(out, view->depthLabeled);
		break;
	}
	case ITMMainEngine::InfiniTAM_IMAGE_SEGMENTATION_MODEL: // segmentation of the model projected into camera frame 
	{
		out->ChangeDims(view->depth->noDims) ;
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::IntToUchar4(out, view->modelLabeled);
		break;
	}
	case ITMMainEngine::InfiniTAM_IMAGE_PROPAGATION_MODEL: // propagation result 
	{
		out->ChangeDims(view->depth->noDims) ;
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::IntToUchar4(out, view->resultPropag);
		break;
	}
	case ITMMainEngine::InfiniTAM_IMAGE_VERTEX_MODEL:
	{
		out->ChangeDims(view->depth->noDims) ;
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::NormalToUchar4(out, trackingState->pointCloud->locations);
		break;
	}
	case ITMMainEngine::InfiniTAM_IMAGE_NORMAL_MODEL:
	{
		out->ChangeDims(view->depth->noDims) ;
		ITMVisualisationEngine<ITMVoxel, ITMVoxelIndex>::NormalToUchar4(out, trackingState->pointCloud->colours);
		break;
	}
	};
}
