...
ImageFileReader::ImageFileReader(const char *calibFilename, const char *rgbImageMask, const char *depthImageMask, const char *semImageMask)
	: ImageSourceEngine(calibFilename)
{
	strncpy(this->rgbImageMask, rgbImageMask, BUF_SIZE);
	strncpy(this->depthImageMask, depthImageMask, BUF_SIZE);
	strncpy(this->semImageMask, semImageMask, BUF_SIZE);
...
ImageFileReader::~ImageFileReader()
{
	...
	if (cached_sem != NULL) delete cached_sem ;
}
...
void ImageFileReader::loadIntoCache(void)
{
	...
	cached_sem = new ITMShortImage(true, false); 
	...
	sprintf(str, semImageMask, currentFrameNo);
	if (!ReadImageFromFile(cached_sem, str)) 
	{
		delete cached_sem; cached_sem = NULL;
		printf("error reading file '%s'\n", str);
	}
}
...
void ImageFileReader::getAllImages(ITMUChar4Image *rgb, ITMShortImage *rawDepth, ITMShortImage *sem) {
	...
	if (cached_sem != NULL) {
		sem->SetFrom(cached_sem, ORUtils::MemoryBlock<short>::CPU_TO_CPU);
		delete cached_sem;
		cached_sem = NULL;
		bUsedCache = true;
	}
	...
	if (!bUsedCache) {
		...
		sprintf(str, semImageMask, currentFrameNo);
		if (!ReadImageFromFile(sem, str)) printf("error reading file '%s'\n", str);
	}
